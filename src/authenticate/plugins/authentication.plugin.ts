...
const handle = <TState extends BaseAuthenticateState>(element: GaHTMLElement, binding: any, store: Store<TState>): void => {
  ...
  const existingUserRols: UserRole[] = store.state.authenticate?.userRoles ?? [];
  ...
  install<TState extends BaseAuthenticateState>(Vue: any, { router, store }: RouterPluginOptions<TState>): void {
    Vue.directive('ga-authentication', {
      bind: (element: GaHTMLElement, binding: any): void => {
        // eslint-disable-next-line @typescript-eslint/camelcase
        element.__ga_authentication_watch__ = store.watch((state: TState) => (state.authenticate?.userRoles ?? []), () => handle(element, binding, store));
    ...
    router.beforeEach((to: Route, _from: Route, next: (to?: RawLocation | false | ((vm: Vue) => any) | void) => void): any => {
      if (!!store.state.authenticate
        && !store.state.authenticate.token
        && (store.state.authenticate.userRoles.length === 0 || store.state.authenticate.userRoles.includes(UserRole.Null))
        && to.path !== '/login') {
        next('/login');
      } else if (to.meta.needsLogin
        && to.meta.allowedUserRoles.filter((allowedUserRole: UserRole) => (store.state.authenticate?.userRoles ?? []).includes(allowedUserRole)).length === 0) {
        next('/no-permission');
      } else {
        next();
      }
